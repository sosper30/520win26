...
## Week 6: Standard Template Libary
...
While Python is dominant for research and prototyping, the production Machine Learning (ML) infrastructure is often C/C++ underneath. 

C++ plays several critical roles in production ML systems:

- Inference engines and serving: TensorFlow, PyTorch, ONNX Runtime all have C++ cores for fast inference. 

- Training frameworks: The computational backends of major frameworks (PyTorch's ATen, TensorFlow's XLA) are C++ for efficiency with hardware accelerators.

- Embedded/edge ML: Mobile devices, robotics, autonomous vehicles, IoT devices run ML in C++ due to resource constraints and real-time requirements.

- High-frequency trading and real-time systems: Where microseconds matter, C++ is the standard for ML-based decision making.

- Computer vision and graphics: OpenCV, many game engines, AR/VR systems use C++ for real-time ML-based vision tasks.

- Optimized libraries: Core numerical libraries (Eigen, Intel MKL, cuBLAS) that Python eventually calls are C/C++.

The rise of ML and generative AI has actually increased demand for skilled C++ programmers, not decreased it for the following reasons:

- ML Infrastructure & Systems where the companies building the foundation of AI need C++ experts:

    - Training infrastructure at scale (distributed systems, GPU optimization)

    - Inference engines that serve billions of requests

    - Custom hardware acceleration (TPUs, NPUs, specialized chips)

    - Model optimization and quantization frameworks

- Generative AI Deployment where the explosion of LLMs and diffusion models creates opportunities:

    - Building efficient serving systems for models like GPT, Claude, Stable Diffusion

    - Developing on-device AI that runs locally (privacy, cost, latency)

- Robotics & Autonomous Systems which fundamentally requires C++:

    - Self-driving cars (perception, planning, control loops)

    - Warehouse automation and delivery robots

    - Humanoid robots (the next big frontier)

    - Drones with real-time vision and decision-making

Therefore, this course has an HW that requires students to implement two ML algorithms in C++. 