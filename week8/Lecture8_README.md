...
Since the final HW includes a C++ implementation of a tool-using LLM agent, it is important to 
understand the terms and concepts associated with Agentic AI. Even though most LLM research and development happens in Python, it will be cool to see what you can do with LLMs in C++. Moreover, C++ is still quite important "under the hood" for production deployment, inference engines, hardware acceleration and training frameworks like PyTorch and TensorFlow.

## What is an AI Agent?

An **AI agent** is a system that can perceive its environment, make decisions, and take actions to achieve specific goals autonomously. Unlike a simple chatbot that only responds to queries, an agent can:
- Break down complex tasks into steps
- Use external tools to gather information or perform actions
- Maintain context across multiple interactions
- Iterate on its approach based on feedback

This pattern is used in:
- **Code assistants**: Execute code, run tests, search documentation
- **Research agents**: Search papers, extract data, analyze results
- **Customer service bots**: Query databases, update records, send emails
- **Data analysis tools**: Load data, perform calculations, create visualizations
- **Personal assistants**: Check calendars, send messages, search the web

A complete AI agent system typically consists of several components:

1. **Large Language Model (LLM)** - The "brain" that processes language, makes decisions, and generates responses
2. **Tools/Functions** - The "hands" that allow the agent to perform actions in the world (calculations, API calls, file operations, etc.)
3. **Agent Loop** - The "nervous system" that orchestrates the decision-making and tool execution process
4. **Memory** - Systems for remembering past interactions and learned information:
   - **Short-term memory**: Current conversation context (what we'll implement in your assignment)
   - **Long-term memory**: Persistent storage of past conversations, learned facts, or user preferences
   - **Working memory**: Intermediate results and reasoning traces during task execution
5. **Planning Module** - Breaks down complex tasks into sub-tasks and determines the sequence of actions
6. **Reflection/Self-Correction** - Ability to evaluate its own outputs and correct mistakes
7. **Knowledge Base** - External information sources the agent can query (databases, documents, web)

<!-- ![LLM Agent Overview](agent-overview.png) -->
![Agent Overview](https://lilianweng.github.io/posts/2023-06-23-agent/agent-overview.png)

**In your assignment**, you'll focus on implementing the core components (LLM, Tools, Agent Loop, and Short-term Memory through conversation history). The other components represent more advanced agent architectures used in production systems.
...
LLM: "I don't have access to real-time weather data..."  ← Limited to training data
```

**Agentic AI with Tools:**
```
User: "What's the weather in Seattle?"
Agent: [Decides to use weather API tool]
Agent: [Calls weather_api("Seattle")]
Tool: Returns {"temp": 55, "condition": "cloudy"}
Agent: "It's currently 55°F and cloudy in Seattle"  ← Real-time, accurate data
```

## Key Concepts and Terms

### 1. **Tool / Function**
A capability that the agent can invoke to perform specific tasks. Tools extend what the LLM can do beyond text generation.

**Examples:**
- Calculator: Performs arithmetic operations
- Web search: Retrieves current information
- Database query: Accesses structured data
- File operations: Reads or writes files
- API calls: Interacts with external services

**In your assignment:** You'll implement mathematical and statistical tools that the LLM can use.

### 2. **Tool Definition / Schema**
A structured description of a tool that tells the LLM:
- What the tool does (description)
- What parameters it accepts (inputs)
- What types those parameters are (number, string, array, etc.)
...
This is like providing a function signature and documentation to the LLM so it knows how to use your tool.

### 3. **Tool Calling / Function Calling**
The mechanism by which an LLM requests to use a tool. Instead of just generating text, the LLM outputs structured data indicating:
- Which tool it wants to use
- What arguments to pass to that tool

**Important:** The LLM doesn't actually execute the tool. It only *requests* that your code execute it.

### 4. **Tool Use / Tool Execution**
The actual execution of the tool by your C++ code. This happens in your application, not in the LLM.

**Flow:**
1. LLM returns: "I want to use `tool_name` with these arguments"
2. Your code extracts this request
3. Your code executes the tool function
4. Your code gets the result
5. Your code sends result back to LLM
6. LLM uses result to generate final response

### 5. **Agent Loop / Agentic Loop**
...
```
User: "Find data from database, then analyze it"

Turn 1:
  LLM: [Calls database_query tool]
  Tool: Returns raw data
  
Turn 2:
  LLM: [Calls analysis tool with the data]
  Tool: Returns analysis results
  
Turn 3:
  LLM: "Based on the data, here's what I found..."
```

The agent autonomously decides to chain multiple tool calls together.
...
This organizational pattern makes it easy to add, remove, or modify tools without changing the core agent logic.

### 7. **Conversation History / Message History**
The complete record of the conversation, including:
- User messages
- Assistant (LLM) messages
- Tool call requests
- Tool results

This context is essential because the LLM only knows what's in the current request.

**Message roles:**
- `user`: Messages from the human
- `assistant`: Messages from the LLM (including tool call requests)
